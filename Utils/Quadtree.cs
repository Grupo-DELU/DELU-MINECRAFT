...
namespace Utils.SpatialTrees.QuadTrees
{
    ...
    public class DataQuadTreeNode<T>
    {
        ...
        public class DataQuadTreeNodeData
        {
            ...
            internal DataQuadTreeNodeData(in Vector2Int point, T data, DataQuadTreeNode<T> node)
            {
                Point = point;
                Data = data;
                Node = node;
        ...
        /// </summary>
        public Vector2Int MinBound { get; private set; }

        /// <summary>
        /// Maximum Bound of this node
        /// </summary>
        public Vector2Int MaxBound { get; private set; }
        ...
        public DataQuadTreeNode(DataQuadTree<T> tree, DataQuadTreeNode<T> parent, in Vector2Int minBound, in Vector2Int maxBound)
        {
            MinBound = minBound;
            MaxBound = maxBound;
            Center = (MinBound / 2.0f) + (MaxBound / 2.0f); // for precision
            Area = (MaxBound.Z - MinBound.Z) * (MaxBound.X - MinBound.X);
            Parent = parent;
            Tree = tree;
        ...
        public Vector2Int GetClosestPointInside(in Vector2Int point)
        {
            return new Vector2Int(
                Math.Clamp(point.Z, MinBound.Z, MaxBound.Z),
                Math.Clamp(point.X, MinBound.X, MaxBound.X)
            );
        }
        ...
        {
            return (MinBound.Z <= point.Z) && (point.Z <= MaxBound.Z) &&
            ...
            if (DataPoints != null)
            {
                ...
                else
                {
                    TopLeftNode = new DataQuadTreeNode<T>(
                        Tree, this, new Vector2Int(MinBound.Z, Center.X), new Vector2Int(Center.Z, MaxBound.X)
                    );
                    ...
                    BottomRightNode = new DataQuadTreeNode<T>(
                        Tree, this, new Vector2Int(Center.Z, MinBound.X), new Vector2Int(MaxBound.Z, Center.X)
                    );
    ...
    public class DataQuadTree<T>
    {
        ...
        public DataQuadTree(
            in Vector2Int minBound, in Vector2Int maxBound,
        ...
        public DataQuadTreeNode<T>.DataQuadTreeNodeData Insert(in Vector2Int point, T data)
        {
        ...
        private class DistanceToNode
        {
            ...
            public DistanceToNode(DataQuadTreeNode<T> node, in Vector2Int point)
            {
                Node = node;
                ManClosestDistance = Vector2Int.Manhattan(Node.GetClosestPointInside(point), point);
            }
        }
        ...
        private class ClosestToNode : Comparer<DistanceToNode>
        {
            ...
            {
                return x.ManClosestDistance.CompareTo(y.ManClosestDistance);
            }
        }
        ...
        public class DistanceToDataPoint
        {
            ...
            {
                DataNode = null;
                ManClosestDistance = maxDominatingDistance;
            }
            ...
            /// <param name="point">Point to use</param>
            public DistanceToDataPoint(DataQuadTreeNode<T>.DataQuadTreeNodeData dataNode, in Vector2Int point)
            {
                DataNode = dataNode;
                ManClosestDistance = Vector2Int.Manhattan(dataNode.Point, point);
            }
        }
        ...
        private void InitClosestNodeHeap(in DataQuadTreeNode<T> node, in Vector2Int point)
        {
        ...
        public DistanceToDataPoint NearestNeighbor(in Vector2Int point)
        {
            ...
            while (!closestNodeHeap.IsEmpty())
            {
                ...
                if (nearestDataPoint.ManClosestDistance < currentNode.ManClosestDistance)
                {
                ...
                if (node.HasChildren())
                {
                    ...
                    if (possibleNode.ManClosestDistance < nearestDataPoint.ManClosestDistance)
                    {
                ...
                else
                {
                    ...
                    for (dataPointIter = 0; dataPointIter < dataPoints.Count; dataPointIter++)
                    {
                        ...
                        if (possibleDataPoint.ManClosestDistance < nearestDataPoint.ManClosestDistance)
                        {
        ...
        public int KNearestNeighbor(in Vector2Int point, int k, in DistanceToDataPoint[] storage)
        {
            if (storage.Length < k)
            {
                k = storage.Length;
            ...
            {
                DistanceToNode currentNode = closestNodeHeap.ExtractDominating();
                if (nearestDataPoints.Count == k && nearestDataPoints.GetTop().ManClosestDistance < currentNode.ManClosestDistance)
                {
                    // Is further away than current best
                    continue;
                }
                ...
                if (node.HasChildren())
                {
                    // Not a leaf
                    possibleNode = new DistanceToNode(node.TopLeftNode, point);
                    if (nearestDataPoints.Count < k || possibleNode.ManClosestDistance < nearestDataPoints.GetTop().ManClosestDistance)
                    {
                        closestNodeHeap.Add(possibleNode);
                    }
                ...
                else
                {
                    ...
                    for (dataPointIter = 0; dataPointIter < dataPoints.Count; dataPointIter++)
                    {
                        ...
                        if (nearestDataPoints.Count == k)
                        {
                            ...
                            if (possibleDataPoint.ManClosestDistance < nearestDataPoints.GetTop().ManClosestDistance)
                            {
